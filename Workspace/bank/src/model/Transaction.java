...
public class Transaction extends model.DebitTransferTransaction implements PersistentTransaction {
	...
	public static PersistentTransaction createTransaction(boolean delayed$Persistence) throws PersistenceException {
		...
		if (delayed$Persistence) {
			result = ConnectionHandler.getTheConnectionHandler().theTransactionFacade.newDelayedTransaction(new java.sql.Timestamp(System.currentTimeMillis()),
					"");
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			if (debitTransfer != null) {
				...
				if (depth > 1) {
					...
					if (forGUI && debitTransfer.hasEssentialFields())
						debitTransfer.toHashtable(allResults, depth, essentialLevel + 1, false, true, tdObserver);
				}
			}
	...
	public Transaction provideCopy() throws PersistenceException {
		...
		result = new Transaction(this.timestamp, this.subject, this.sender, this.state, this.subService, this.This, this.debitTransfer, this.getId());
	...
	public Transaction(java.sql.Timestamp timestamp, String subject, PersistentAccount sender, PersistentDebitTransferState state, SubjInterface subService,
			PersistentDebitTransferTransaction This, PersistentDebitTransferListe debitTransfer, long id) throws persistence.PersistenceException {
		/*
		 * Shall not be used by clients for object construction! Use static
		 * create operation instead!
		 */
		super((java.sql.Timestamp) timestamp, (String) subject, (PersistentAccount) sender, (PersistentDebitTransferState) state, (SubjInterface) subService,
				(PersistentDebitTransferTransaction) This, id);
	...
	public void store() throws PersistenceException {
		if (!this.isDelayed$Persistence())
			return;
		if (this.getClassId() == 146)
			ConnectionHandler.getTheConnectionHandler().theTransactionFacade.newTransaction(new java.sql.Timestamp(System.currentTimeMillis()), "",
					this.getId());
		super.store();
	...
	public void setDebitTransfer(PersistentDebitTransferListe newValue) throws PersistenceException {
		...
		if (newValue.equals(this.debitTransfer))
			return;
	...
	public int getLeafInfo() throws PersistenceException {
		if (this.getState() != null)
			return 1;
		if (this.getDebitTransfer() != null && this.getDebitTransfer().getTheObject().getLeafInfo() != 0)
			return 1;
		return 0;
	}
	...
	public void addToTransaction(final DebitTransferSearchList debitTransfer) throws PersistenceException {
	...
	public PersistentBooleanValue contains(final PersistentDebitTransferTransaction debitTransfer) throws PersistenceException {
	...
	public PersistentTransaction mirror() throws model.AccountSearchException, PersistenceException {
	...
	public void removeFromTransaction(final DebitTransferSearchList debitTransfer) throws PersistenceException {
		debitTransfer.applyToAll(new Procdure<PersistentDebitTransfer>() {
			@Override
	...
	public PersistentDebitTransferTransaction copy() throws PersistenceException {
	...
	public PersistentDebitTransferTransaction executeImplementation() throws model.ExecuteException, PersistenceException {
